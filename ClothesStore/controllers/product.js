...
exports.getCreatePro = (req, res, next) => {
    ...
    if (!req.isAuthenticated()) {
        const categories = Categories.find().then(cat => {
            const labels = Label.findOne().then(label => {
              const listlabel = label.list;
              res.render("create-prod", {
                title: "Thêm sản phẩm",
                message: `${message}`,
                user: req.user,
                cartProduct: cartProduct,
...
exports.PostStorePro = (req, res, next) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    var sub = "";
    if (fields.ProductType == "Phụ Kiện") {
      sub = fields.ProductSubTypePhuKien;
    };
    if (fields.ProductType == "Áo") {
      sub = fields.ProductSubTypeAo;
    };
    ...
    var sale = {
      status:(fields.ProductSale) ? true : false,
      percent: (fields.ProductSale) ? fields.ProductSale : 0,
    };
    var newproduct = {
      ...
      size: size,
      productType: type,
      color: fields.ProductColor.split("\r\n"),
      isSale: sale,
      labels: fields.ProductLabel,
      materials: fields.ProductMaterial.split("\r\n"),
    };
